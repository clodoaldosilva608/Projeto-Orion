...
export default async function ProdutosPage() {
  const rows: ProductWithFeatures[] = await prisma.product.findMany({
    where: { status: "active" },
      ...
      <main className="max-w-6xl mx-auto px-4 lg:px-8 py-12">
        <div className="text-center max-w-2xl mx-auto mb-12">
          ...
          <p className="mt-4 text-muted">
            Modelos profissionais gerados pela Orion, prontos para implantação
            imediata.
          </p>
        </div>
        ...
        {rows.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {rows.map((p) => {
              const features = readFeatures(p.features);
              const category =
              ...
              return (
                <div
                  key={p.id}
                  className="glass-card glass-card-hover p-6 lg:p-8 h-full flex flex-col"
                >
                  <div className="flex-1 rounded-xl flex items-center justify-center min-h-[180px] relative overflow-hidden mb-6"
                    style={{
                      background: `linear-gradient(135deg, ${p.iconColor}cc, ${p.iconColor}66)`,
                    }}
                  >
                  ...
                  {p.demoUrl && (
                    <p className="mt-3 text-[11px] text-muted/70">
                      Demo pública: {p.demoUrl.replace(/^https?:\/\//, "")}
                    </p>
                  )}
                </div>
              );
